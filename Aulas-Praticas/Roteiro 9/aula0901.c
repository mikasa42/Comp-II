...
$Date: 2021/10/16 16:40:28 $
$Log: aula0901.c,v $
Revision 1.2  2021/10/16 16:40:28  mikaela.alves
roteiro 9 - arquivo com todas as funcoes

Revision 1.1  2021/10/12 22:40:46  mikaela.alves
Initial revision
...
ConverterArquivoFormatoUnixParaFormatoDos (char *nomeArquivo, char *nomeConvertido)
{
	...
	if (nomeConvertido == NULL)
	{
		...
		backup = (char *) malloc (sizeof (char[strlen(nomeArquivo)+25]));
		if ( backup == NULL)
			return memoriaInsuficiente;

	}
	else
		arquivoEscrita = fopen (nomeConvertido, "w");	
	...
	if (nomeConvertido == NULL)
	{
		...
		snprintf (backup,strlen(nomeArquivo)+25,"%sbackup-%04d%02d%02d_%02d%02d%02d",
				nomeArquivo,
		...
		rename (nomeArquivo,backup);
		rename (temporario,nomeArquivo);
	}
